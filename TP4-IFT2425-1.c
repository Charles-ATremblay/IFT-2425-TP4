...
// author  : Khaoula Benzaarit (20176850)
// date    : 
// version : 1.0
// language: C++
// note    :
...
double prim_x(double x_t, double y_t, double z_t) {
    return -R * z_t + ((X_1-x_t) / pow(CARRE(X_1-x_t) + CARRE(Y_1-y_t) + CARRE(D), 3.0/2.0))
           + ((X_2-x_t) / pow(CARRE(X_2-x_t) + CARRE(Y_2-y_t) + CARRE(D), 3.0/2.0))
           + ((X_3-x_t) / pow(CARRE(X_3-x_t) + CARRE(Y_3-y_t) + CARRE(D), 3.0/2.0))
           - C * x_t;
}

double prim_y(double x_t, double y_t, double z_t) {
    return -R * z_t + ((Y_1-y_t) / pow(CARRE(X_1-x_t) + CARRE(Y_1-y_t) + CARRE(D), 3.0/2.0))
           + ((Y_2-y_t) / pow(CARRE(X_2-x_t) + CARRE(Y_2-y_t) + CARRE(D), 3.0/2.0))
           + ((Y_3-y_t) / pow(CARRE(X_3-x_t) + CARRE(Y_3-y_t) + CARRE(D), 3.0/2.0))
           - C * y_t;
}

double runge_kutta_fehlberg_X(double x, double y, double z) {
...
double runge_kutta_fehlberg_Y(double x, double y, double z) {
    double k1,k2,k3,k4,k5,k6;
    k1 = H * df(x, y, z);
...
double runge_kutta_fehlberg_Xx(double x, double y, double z) {
    ...
    k1 = H * prim_x(x, y, z);
    k2 = H * prim_x(x + H/4.0, y, z + k1/4.0);
    k3 = H * prim_x(x + 3*H/8.0, y, z + 3*k1/32.0 + 9*k2/32.0);
    k4 = H * prim_x(x + 12*H/13.0, y, z + 1932*k1/2197.0 - 7200*k2/2197.0 + 7296*k3/2197.0);
    k5 = H * prim_x(x + H, y, z + 439*k1/216.0 - 8*k2 + 3680*k3/513.0 - 845*k4/4104.0);
    k6 = H * prim_x(x + H/2.0, y, z - 8*k1/27.0 + 2*k2 - 3544*k3/2565.0 + 1859*k4/4104.0 - 11*k5/40.0);
...
double runge_kutta_fehlberg_Yy(double x, double y, double z) {
    ...
    k1 = H * prim_y(x, y, z);
    k2 = H * prim_y(x, y + H/4.0, z + k1/4.0);
    k3 = H * prim_y(x, y + 3*H/8.0, z + 3*k1/32.0 + 9*k2/32.0);
    k4 = H * prim_y(x, y + 12*H/13.0, z + 1932*k1/2197.0 - 7200*k2/2197.0 + 7296*k3/2197.0);
    k5 = H * prim_y(x, y + H, z + 439*k1/216.0 - 8*k2 + 3680*k3/513.0 - 845*k4/4104.0);
    k6 = H * prim_y(x, y + H/2.0, z - 8*k1/27.0 + 2*k2 - 3544*k3/2565.0 + 1859*k4/4104.0 - 11*k5/40.0);
...
double x = X_1_INI, y = X_3_INI, vitesse_x = X_2_INI, vitesse_y = X_4_INI;
double tvx, tvy, tx, ty;
for(int k = 0; k < NB_INTERV; k++) {
    // Compute temporary variables using Runge-Kutta-Fehlberg method
    tvx = runge_kutta_fehlberg_X(x, y, vitesse_x);
    tvy = runge_kutta_fehlberg_Y(x, y, vitesse_y);
    tx = runge_kutta_fehlberg_Xx(x, y, vitesse_x);
    ty = runge_kutta_fehlberg_Yy(x, y, vitesse_y);

    // Update position and velocity using temporary variables
    x = tvx;
    vitesse_x = tx;
    y = tvy;
    vitesse_y = ty;

    // Store current position in matrix
    MatPts[k][0] = x;
    MatPts[k][1] = y;
}
