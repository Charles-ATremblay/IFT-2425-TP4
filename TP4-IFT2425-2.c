...
// author  : Charles-Antoine Tremblay (20123754)
// date    :
// version : 1.0
// language: C++
// note    :
...
Window fabrique_window(char *nom_fen, int x, int y, int width, int height, int zoom)
{
  ...
  XSelectInput(display, win, ExposureMask | KeyPressMask | ButtonPressMask | ButtonReleaseMask | ButtonMotionMask | PointerMotionHintMask | StructureNotifyMask);
...
double g = 9.81; // Accélération due à la gravité
double L = 1.0;  // Longueur de la corde

// Paramètres de simulation
double h = 0.1;       // Pas de temps
double t0 = 0.0;      // Temps initial
double tfinal = 20.0; // Temps final

// Paramètres de convergence
double seuil = 0.5;     // Seuil de convergence
int nb_iterations = 20; // Nombre d'itérations nécessaires pour considérer que le pendule a convergé

// Positions des aimants
double x_aimant1 = 0.0;
double y_aimant1 = 0.0;
double x_aimant2 = -0.5;
double y_aimant2 = -sqrt(3.0) / 2.0;
double x_aimant3 = 0.5;
double y_aimant3 = -sqrt(3.0) / 2.0;

// Fonction d'évolution du pendule
void pendule(double t, double *y, double *dy)
{
  dy[0] = y[1];
  dy[1] = -(g / L) * sin(y[0]);
}

// Fonction pour calculer la distance entre le pendule et les aimants
double distance_aimants(double x, double y)
{
  double dist1 = fabs(sqrt(pow(x - x_aimant1, 2.0) + pow(y - y_aimant1, 2.0)) - seuil);
  double dist2 = fabs(sqrt(pow(x - x_aimant2, 2.0) + pow(y - y_aimant2, 2.0)) - seuil);
  double dist3 = fabs(sqrt(pow(x - x_aimant3, 2.0) + pow(y - y_aimant3, 2.0)) - seuil);
...
int main(int argc, char **argv)
{
  ...
  double y[2] = {PI / 2.0, 0.0};     // Conditions initiales
  double dy[2];                      // Dérivées
  double t = t0;                     // Temps courant
  int nb_consecutifs = 0;            // Nombre d'itérations consécutives où le pendule est proche d'un aimant
  double x = L * sin(y[0]);          // Position x du pendule
  double y_pendule = -L * cos(y[0]); // Position y du pendule

  // Boucle de simulation
  while (t <= tfinal && nb_consecutifs < nb_iterations)
  {
    // Calcul des dérivées
    pendule(t, y, dy);
    ...
    // Calcul de la distance entre le pendule et les aimants
    double dist = distance_aimants(x, y_pendule);

    // Vérification si le pendule est proche d'un aimant
    if (dist < seuil)
    {
      nb_consecutifs++;
    }
    ...
    // Affichage de la vitesse de convergence
    printf("Temps : %f, Distance : %f, Nb itérations : %d\n", t, dist, nb_consecutifs);
  ...
  if (nb_consecutifs >= nb_iterations)
  {
    printf("Le pendule a convergé en %f secondes\n", t - nb_iterations * h);
  }
  else
  {
    printf("Le pendule n'a pas convergé en %f secondes\n", tfinal);
  }
